    ...
    var calendar = flatpickr("#date-btn", {
        ...
        onChange: function(selectedDates, dateStr, instance) {
            if (tripMethod === "round" && selectedDates.length === 2) {
                ...
                document.getElementById('date-btn').innerText = 
    ...
    tripMethods.forEach(function (radio) {
        radio.addEventListener('change', function () {
            if (this.value === "round") {
                ...
                document.getElementById('date-btn').innerText = "가는 날 ~ 오는 날"; // 텍스트 변경
            } else if (this.value === "one-way") {
                tripMethod = "one-way";
                calendar.set("mode", "single"); // 편도일 때는 한 날짜 선택
                document.getElementById('date-btn').innerText = "가는 날"; // 텍스트 변경
            }
        });
    });
});

//flatpickr를 적용하는 공통 함수
function initializeFlatpickr(selector) {
    flatpickr(selector, {
        locale: "ko", // 한국어 설정
        dateFormat: "Y-m-d", // 날짜 형식
        showMonths: 1, // 한 달치 달력만 표시
        allowInput: true // 사용자가 직접 날짜 입력 가능
    });
}

// DOMContentLoaded 이벤트 후에 flatpickr 적용
document.addEventListener('DOMContentLoaded', function() {
    // 조회 출발일 버튼에 flatpickr 적용
    initializeFlatpickr("#date");

    // 체크인 출발일 버튼에 flatpickr 적용
    initializeFlatpickr("#cdate");
});
...
<form action="${pageContext.request.contextPath}/flights/search" method="get">
								...
								<div id="trip-methods" class="trip-methods">
									<input type="radio" name="t_methods" value="round"
										id="round-trip" checked> <label for="round-trip">ffff복</label>
