...
<head>
	...
	<script>
		document.addEventListener('DOMContentLoaded', function() {
			...
			document.getElementById('booking').parentNode.classList.add('active'); // 항공권예매 버튼에 active 클래스 추가
		...
		function sizedown(n) {
			...
			if(num != 11) {
				var s1 = setInterval(function() {   
					...
					document.getElementsByClassName("inner")[n].style.color = "#1f0c59";
		...
		function init(my, n) {
			if(my.value.trim().length == 0) {
				...
				var s2 = setInterval(function() {   
					...
					document.getElementsByClassName("inner")[n].style.color = "#333";
					if(n == 0) {
						document.getElementsByClassName("inner")[n].textContent = "아이디";
					} else {
						document.getElementsByClassName("inner")[n].textContent = "비밀번호";
		...
		function useridSearch() {
			var son = open("usForm", "", "width=400,height=300");
			son.moveTo(200,200);
		}
		
		function pwdSearch() {
			var son = open("psForm", "", "width=400,height=370");
			son.moveTo(200,200);
		}
		
		// 출발지 선택 시
		function selectDeparture(airportCode, city) {
			document.getElementById('from-text').textContent = airportCode;
			document.getElementById('departure-text').textContent = city;
			document.getElementById('from-hidden').value = airportCode; // hidden 필드에 값 설정
			console.log("출발지 설정: " + airportCode); // 디버그용 로그
				...
				<div class="air_contentbox">
					...
					<div class="air_widget">
						...
						<style>
							...
							#select_number, #check-in_number, #select_date, #check-in_date, #select_sung, #check-in_sung, #select_name, #check-in_name {
								display: flex;
								flex-direction: column;
