...
<script>
	...
	function openPopup(type) {
	    ...
	    if (type === 'passenger') {
	        ...
	        loadDeparture();  // 팝업이 열릴 때 데이터를 로드
	...
	function loadDeparture() {
	    ...
	    xhr.onreadystatechange = function() {
	        if (xhr.readyState === 4 && xhr.status === 200) {
	            ...
	            // airport-list 요소가 존재하는지 확인
	            if (airportList) {
	                airportList.innerHTML = ''; // 기존 목록 비우기
	
	                // 기본값 설정 (DB에서 불러온 데이터 중 하나를 선택)
	                var defaultAirport = data.find(airport => airport.airportCode === 'SEL'); // 기본값을 SEL로 설정
	                ...
	                if (defaultAirport) {
	                    ...
	                    // 만약 defaultAirport가 없으면 기본적으로 SEL 서울을 설정
